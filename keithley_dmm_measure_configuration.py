...
import keithley_dmm_constants as _dmmconst
...
class MeasureConfiguration:
    ...
    def autorangerebound(self):
        ...
        retconstval = None
        if "ON" in response:
            retconstval = _dmmconst.ON
        else:
            retconstval = _dmmconst.OFF
        return retconstval
    ...
    class ConfigList:
        ...
        def storefunc(self, list_name, function, index=None):
            ...
            """
            function_str = None
            if function == _dmmconst.FUNC_DC_VOLTAGE:
                function_str = "smu.FUNC_DC_VOLTAGE"
            elif function == _dmmconst.FUNC_DC_CURRENT:
                function_str = "smu.FUNC_DC_VOLTAGE"
            elif function == _dmmconst.FUNC_RESISTANCE:
    ...
    def function(self):
        ...
        if "VOLTAGE" in response:
            retconstval = _dmmconst.FUNC_DC_VOLTAGE
        elif "CURRENT" in response:
            retconstval = _dmmconst.FUNC_DC_CURRENT
        elif "RESISTANCE" in response:
            retconstval = _dmmconst.FUNC_RESISTANCE
        return retconstval
    ...
    class Filter:
        ...
        def type(self):
            ...
            if "MOVING" in filtertype:
                retconstval = _dmmconst.FILTER_MOVING_AVG
            else:
                retconstval = _dmmconst.FILTER_REPEAT_AVG
            return retconstval
        ...
        def type(self, filtertype):
            ...
            :return:
            """
            if filtertype == _dmmconst.FILTER_MOVING_AVG:
    ...
    class Limit:
        ...
        def audible(self, limit_number, state=None):
            ...
            if state is None:
                ...
                if "NONE" in audible:
                    retconstval = _dmmconst.AUDIBLE_NONE
                elif "FAIL" in audible:
                    retconstval = _dmmconst.AUDIBLE_FAIL
                elif "PASS" in audible:
                    retconstval = _dmmconst.AUDIBLE_PASS
            else:
                if state == _dmmconst.AUDIBLE_NONE:
                    self._mycomms.write(f"smu.measure.limit[{limit_number}].\
                        audible = smu.AUDIBLE_NONE")
                elif state == _dmmconst.AUDIBLE_FAIL:
                    self._mycomms.write(f"smu.measure.limit[{limit_number}].\
                        audible = smu.AUDIBLE_FAIL")
                elif state == _dmmconst.AUDIBLE_PASS:
                    self._mycomms.write(f"smu.measure.limit[{limit_number}].\
        ...
        def fail(self, limit_number):
            ...
            if "NONE" in audible:
                retconstval = _dmmconst.FAIL_NONE
            elif "HIGH" in audible:
                retconstval = _dmmconst.FAIL_HIGH
            elif "LOW" in audible:
                retconstval = _dmmconst.FAIL_LOW
            elif "BOTH" in audible:
                retconstval = _dmmconst.FAIL_BOTH
    ...
    class Math:
        ...
        def format(self, operation):
            ...
            if operation is None:
                ...
                if "MXB" in operation:
                    retconstval = _dmmconst.MATH_MXB
                elif "PERCENT" in operation:
                    retconstval = _dmmconst.MATH_PERCENT
                elif "RECPROCAL" in operation:
                    retconstval = _dmmconst.MATH_RECIPROCAL
            else:
                if operation == _dmmconst.MATH_MXB:
                    self._mycomms.write("smu.measure.math.format = \
                        smu.MATH_MXB")
                elif operation == _dmmconst.MATH_PERCENT:
                    self._mycomms.write("smu.measure.math.format = \
                        smu.PERCENT")
                elif operation == _dmmconst.MATH_RECIPROCAL:
                    self._mycomms.write("smu.measure.math.format = \
                        smu.RECIPROCAL")
            return retconstval
    ...
    @sense.setter
    def sense(self, sense_type=_dmmconst.SENSE_2WIRE):

        sense_string = ""
        if sense_type is _dmmconst.SENSE_2WIRE:
    ...
    def unit(self):
        ...
        if "AMP" in unit_of_measure:
            return _dmmconst.UNIT_AMP
        elif "OHM" in unit_of_measure:
            return _dmmconst.UNIT_OHM
        elif "VOLT" in unit_of_measure:
            return _dmmconst.UNIT_VOLT
        elif "WATT" in unit_of_measure:
            return _dmmconst.UNIT_WATT
        return 0

    @unit.setter
    def unit(self, unit_of_measure=_dmmconst.UNIT_AMP):
        """
        ...
        """
        unit_string = ""
        if unit_of_measure is _dmmconst.UNIT_AMP:
            unit_string = "smu.UNIT_AMP"
        elif unit_of_measure is _dmmconst.UNIT_OHM:
            unit_string = "smu.UNIT_OHM"
        elif unit_of_measure is _dmmconst.UNIT_VOLT:
            unit_string = "smu.UNIT_VOLT"
        elif unit_of_measure is _dmmconst.UNIT_WATT:
